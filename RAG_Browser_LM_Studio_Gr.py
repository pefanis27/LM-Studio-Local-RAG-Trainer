...
"""
Single-file εφαρμογή RAG για τοπική επικοινωνία με LM Studio.

Κύριες δυνατότητες:
- Αυτόματο άνοιγμα της Streamlit ιστοσελίδας στον default browser
- Επιλογή LLM από λίστα και εκκίνηση service + φόρτωση μοντέλου
- Προτεραιότητα σε background service χωρίς ορατό άνοιγμα του LM Studio
- Κυλιόμενο answer panel με σωστή μορφοποίηση markdown και copy button
- Επισύναψη αρχείων στην ερώτηση (PDF, DOCX, κώδικας, κείμενο κ.ά.)
- Δημιουργία και χρήση συλλογών γνώσης με αναζήτηση RAG
- Πιο καθαρή εμφάνιση πηγών και συλλογών
"""
...
if __name__ == "__main__" and os.environ.get(STREAMLIT_ENV_FLAG) != "1":
    ...
    print("🚀 Εκκίνηση single-file RAG εφαρμογής μέσω Streamlit...")
    print(f"📂 Αρχείο: {os.path.abspath(__file__)}")
    print(f"🐍 Python: {sys.executable}")
    print(f"🌐 Το app θα ανοίξει αυτόματα στο: http://{STREAMLIT_HOST}:{STREAMLIT_SERVER_PORT}")
    print()
    ...
    try:
        ...
        print("❌ Δεν βρέθηκε η εντολή streamlit. Κάνε εγκατάσταση με:")
        print("   pip install streamlit")
        raise SystemExit(1)
    except KeyboardInterrupt:
        print("\n⛔ Η εκτέλεση διακόπηκε από τον χρήστη.")
...
APP_SUBTITLE = "Τοπικό RAG με LM Studio πάνω στα δικά σου αρχεία"
...
NO_COLLECTION_OPTION = "📎 Χωρίς συλλογή (μόνο συνημμένα)"
...
def build_attachment_context(
    ...
    *,
    ...
    for doc_offset, document in enumerate(documents, start=start_index):
        ...
        if truncated:
            snippet = snippet.rstrip() + "\n\n[... το συνημμένο περικόπηκε για να χωρέσει στο prompt ...]"

        context_blocks.append(
            f"[ΠΗΓΗ {doc_offset}] αρχείο={document.filename} | kind=attachment | score=1.0000\n{snippet}"
        )
        sources.append(
            {
                "score": 1.0,
...
def load_question_attachments(uploaded_files: Iterable) -> tuple[list[LoadedDocument], list[str]]:
    ...
    for uploaded in uploaded_files or []:
        if not is_supported(uploaded.name):
            warnings.append(f"Το αρχείο '{uploaded.name}' αγνοήθηκε επειδή ο τύπος του δεν υποστηρίζεται ακόμη.")
            continue
        try:
...
def launch_lmstudio_app() -> str | None:
    for command in _candidate_lmstudio_app_commands():
        ...
        if started:
            suffix = " στο background" if OPEN_LMSTUDIO_APP_IN_BACKGROUND else ""
            return f"Έγινε προσπάθεια ανοίγματος του LM Studio{suffix}: {' '.join(command)}"
    return None
...
def start_lmstudio_server(base_url: str) -> str | None:
    ...
    for base_cmd in _candidate_lms_commands():
        ...
        if _spawn_background_process(command):
            return f"Έγινε προσπάθεια εκκίνησης του service: {' '.join(command)}"
    return None
...
def ensure_lmstudio_online(
    ...
    if is_lmstudio_api_reachable(base_url, api_key=api_key, timeout=4):
        report["ok"] = True
        report["message"] = "Το LM Studio server απαντά ήδη."
    ...
    while time.time() < early_deadline:
        if is_lmstudio_api_reachable(base_url, api_key=api_key, timeout=4):
            report["ok"] = True
            report["message"] = f"Το LM Studio service ξεκίνησε χωρίς να ανοίξει το παράθυρο της εφαρμογής στο προσκήνιο ({host}:{port})."
            report["actions"].append(report["message"])
            return report
        time.sleep(1.0)
    ...
    while time.time() < deadline:
        if is_lmstudio_api_reachable(base_url, api_key=api_key, timeout=4):
            report["ok"] = True
            report["message"] = f"Το LM Studio είναι διαθέσιμο στο {host}:{port}."
            report["actions"].append(report["message"])
            return report
        time.sleep(1.0)

    report["message"] = (
        "Δεν κατέστη δυνατό να ενεργοποιηθεί αυτόματα το LM Studio service στο background. "
        "Έλεγξε αν το LM Studio είναι εγκατεστημένο και αν υπάρχει το CLI 'lms'."
    )
    if not report["actions"]:
        report["actions"].append("Δεν βρέθηκε έγκυρη εντολή ή executable για αυτόματη εκκίνηση.")
    return report
...
class LMStudioClient:
    ...
    def _get(self, path: str) -> Any:
        ...
        try:
            ...
            raise LMStudioError(f"Αποτυχία GET στο LM Studio: {exc}") from exc
        ...
        try:
            ...
            try:
                if response is not None:
                    details = f" | Απόκριση: {response.text[:500]}"
            except Exception:
                pass
            raise LMStudioError(f"Αποτυχία POST στο LM Studio: {exc}{details}") from exc
    ...
    def create_embeddings(self, texts: list[str], model: str) -> list[list[float]]:
        ...
        if not vectors:
            raise LMStudioError("Το LM Studio δεν επέστρεψε embeddings.")
    ...
    def chat(
        ...
        if not choices:
            raise LMStudioError("Το LM Studio δεν επέστρεψε απάντηση.")
...
class RAGEngine:
    ...
    def build_collection(
        ...
        if not documents:
            raise RAGEngineError("Δεν δόθηκαν αρχεία για δημιουργία γνώσης.")

        chunks = build_chunks(documents, chunk_size=chunk_size, chunk_overlap=chunk_overlap)
        if not chunks:
            raise RAGEngineError("Δεν προέκυψαν chunks από τα δεδομένα σου.")
    ...
    def answer_question(
        ...
        if collection_name:
            if not embedding_model:
                raise RAGEngineError("Απαιτείται embedding model όταν χρησιμοποιείται συλλογή γνώσης.")
            ...
            for index, hit in enumerate(hits, start=1):
                context_blocks.append(
                    f"[ΠΗΓΗ {index}] αρχείο={hit.filename} | chunk={hit.chunk_index} | score={hit.score:.4f}\n{hit.text}"
                )
        ...
        if not context_blocks:
            raise RAGEngineError(
                "Δεν βρέθηκαν σχετικά αποσπάσματα ούτε δόθηκαν αναγνώσιμα συνημμένα για να απαντήσω."
            )

        system_prompt = (
            "Είσαι βοηθός RAG που απαντά αποκλειστικά με βάση το παρεχόμενο context. "
            "Αν το context δεν αρκεί, πες το καθαρά. "
            "Να απαντάς στα Ελληνικά, με σαφήνεια και χωρίς να επινοείς πληροφορίες. "
            "Όταν χρησιμοποιείς στοιχεία από το context, να αναφέρεις παραπομπές με τη μορφή [ΠΗΓΗ 1], [ΠΗΓΗ 2]."
        )
        ...
        user_prompt = (
            ...
            f"Ερώτηση χρήστη:\n{question}\n\n"
            "Οδηγίες:\n"
            "1. Απάντησε μόνο με όσα στηρίζονται στο context.\n"
            "2. Αν λείπει πληροφορία, δήλωσέ το.\n"
            "3. Βάλε παραπομπές [ΠΗΓΗ x] μέσα στην απάντηση.\n"
            "4. Χρησιμοποίησε bullets ή μικρές ενότητες όταν βοηθά τη σαφήνεια.\n"
            "5. Αν υπάρχουν συνημμένα, μπορείς να τα αξιοποιήσεις σαν επιπλέον πηγές context."
        )
...
def refresh_models() -> None:
    ...
    try:
        ...
        if not models:
            errors.append("Το API απάντησε αλλά δεν επέστρεψε μοντέλα.")
    except Exception as exc:
...
def show_sources(sources: list[dict]) -> None:
    for idx, src in enumerate(sources, start=1):
        ...
        source_label = "Συνημμένο" if source_type == "attachment" else "Συλλογή"
        chunk_index = src.get("chunk_index", 0)
        score = float(src.get("score", 0.0))
        with st.expander(
            f"ΠΗΓΗ {idx} — {src['filename']} — {source_label} — chunk {chunk_index} — score {score:.4f}"
        ):
...
def resolve_model(
    *,
    ...
    if not available:
        raise LMStudioError(
            f"Δεν βρέθηκε διαθέσιμο model τύπου '{model_type}' στο LM Studio. "
            "Κάνε refresh ή κατέβασε/πρόσθεσε το κατάλληλο model."
        )
    ...
    if selected_identifier and selected_identifier != AUTO_OPTION:
        ...
        if model and model.type == model_type:
            return model, "Χρησιμοποιείται το model που επέλεξες χειροκίνητα."
        raise LMStudioError(f"Το επιλεγμένο model '{selected_identifier}' δεν είναι διαθέσιμο ως {model_type} model.")
    ...
    if model_type == "embedding" and collection_name:
        try:
            ...
            if model and model.type == "embedding":
                return model, "Βρέθηκε και προτιμήθηκε το embedding model που χρησιμοποιήθηκε για τη συλλογή."
        except Exception:
            pass
    ...
    if loaded:
        ...
        return loaded[0], "Βρέθηκε ήδη φορτωμένο κατάλληλο model στο LM Studio."

    if model_type == "embedding":
        sorted_available = sorted(available, key=rank_embedding_candidate)
        return sorted_available[0], "Δεν υπήρχε χειροκίνητη επιλογή· χρησιμοποιήθηκε αυτόματο fallback embedding model."

    sorted_available = sorted(available, key=lambda m: (m.display_name or m.identifier).lower())
    return sorted_available[0], "Δεν υπήρχε χειροκίνητη επιλογή· χρησιμοποιήθηκε αυτόματο fallback LLM."
...
def ensure_model_loaded(client: LMStudioClient, model: AvailableModel) -> str:
    ...
    if not startup_report.get("ok"):
        raise LMStudioError(startup_report.get("message", "Το LM Studio service δεν είναι διαθέσιμο."))
    ...
    if refreshed_before_load and refreshed_before_load.is_loaded:
        return f"Το model '{model.identifier}' είναι ήδη φορτωμένο."
    ...
    if refreshed and refreshed.is_loaded:
        return f"Το model '{model.identifier}' φορτώθηκε επιτυχώς."

    return f"Έγινε αίτημα φόρτωσης για το model '{model.identifier}'."
...
def render_hero(collections_count: int, llm_count: int, embedding_count: int) -> None:
    st.markdown(
        ...
        <div class="hero-card">
            ...
            <div class="hero-subtitle">
                Φτιάξε τοπική γνώση από τα αρχεία σου και πάρε απαντήσεις RAG από το LM Studio,
                μέσα από καθαρότερο web περιβάλλον.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with a:
        st.markdown(
            f"<div class='mini-card'><div class='mini-card-title'>Συλλογές γνώσης</div><div class='mini-card-value'>{collections_count}</div></div>",
            unsafe_allow_html=True,
        )
    with b:
        st.markdown(
...
def render_status_strip(base_url: str, collections_count: int) -> None:
    st.markdown(
        ...
        <div class="section-note">
            ...
            <span class="status-pill">Συλλογές: {collections_count}</span>
...
def build_answer_panel_html(answer_text: str, dom_id: str) -> str:
    ...
    <div class="rag-answer-inline" id="answer_wrap_{dom_id}">
        <div class="rag-answer-inline-toolbar">
            <div class="rag-answer-inline-note">Κυλιόμενο πλαίσιο απάντησης με μορφοποίηση markdown</div>
        </div>
...
def render_answer_panel(result: dict | None, panel_key_prefix: str = "latest") -> None:
    if not result:
        st.info("Δεν υπάρχει ακόμη απάντηση. Κάνε μια ερώτηση από την καρτέλα 'Ερώτηση'.")
        return
    ...
    st.markdown(
        ...
        <div class="answer-shell">
            <div class="answer-header">
                <div class="answer-title">📋 Τελευταία απάντηση</div>
                <div class="answer-badge">Μορφοποιημένη προβολή</div>
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    if question:
        st.caption(f"Ερώτηση: {question}")

    info_cols = st.columns(5)
    info_cols[0].metric("Συλλογή", metadata.get("collection", "-"))
    info_cols[1].metric("LLM", metadata.get("llm_model", "-"))
    info_cols[2].metric("Embedding", metadata.get("embedding_model", "-"))
    info_cols[3].metric("Πηγές", str(len(result.get("sources", []))))
    info_cols[4].metric("Συνημμένα", str(metadata.get("attachments_count", 0)))
    ...
    if attachment_names:
        st.caption("Συνημμένα ερώτησης: " + ", ".join(attachment_names))

    with st.expander("Κείμενο για αντιγραφή", expanded=False):
        st.text_area(
            "Απλό κείμενο απάντησης",
            value=answer_text,
            height=140,
    ...
    st.markdown("### Πηγές που χρησιμοποιήθηκαν")
...
st.markdown(
    ...
    <div class="footer-note">
        Η μέθοδος <strong>RAG δεν κάνει fine-tuning</strong> στο μοντέλο. Δημιουργεί μια τοπική βάση γνώσης από τα αρχεία σου
        και χρησιμοποιεί το μοντέλο του LM Studio για να απαντά πάνω σε αυτά με παραπομπές.
    </div>
    """,
    unsafe_allow_html=True,
)
...
with st.sidebar:
    st.header("⚙️ Σύνδεση με LM Studio")
    ...
    with start_col:
        if st.button("🚀 Εκκίνηση LM Studio", type="primary", use_container_width=True):
            ...
            if report.get("ok"):
                try:
                    ...
                    pass
                st.success(report.get("message", "Το LM Studio είναι έτοιμο."))
            else:
                st.warning(report.get("message", "Δεν ήταν δυνατή η αυτόματη εκκίνηση."))

    with check_col:
        if st.button("🔎 Έλεγχος / σύνδεση", use_container_width=True):
            report = ensure_lmstudio_online(
                st.session_state.base_url,
                api_key=st.session_state.api_key,
                wait_timeout=15,
            ...
            if report.get("ok"):
                try:
                    ...
                    pass
                st.success("Το LM Studio απαντά κανονικά.")
            else:
                st.error(report.get("message", "Αποτυχία σύνδεσης με το LM Studio."))

    if st.button("🔄 Ανανέωση μοντέλων", use_container_width=True):
        try:
            refresh_models()
            st.success("Η λίστα μοντέλων ανανεώθηκε.")
        except Exception as exc:
            st.error(f"Αποτυχία ανάκτησης μοντέλων: {exc}")

    report = st.session_state.lmstudio_boot_report
    if isinstance(report, dict):
        status_label = "🟢 Service ενεργό" if report.get("ok") else "🟠 Service εκτός / άγνωστο"
        st.markdown(
            f"""
        ...
        if actions:
            st.caption("Τελευταίες ενέργειες εκκίνησης:")
            for action in actions:
                st.write(f"- {action}")

    st.divider()
    st.subheader("🎯 Επιλογή μοντέλου service")
    ...
    st.selectbox(
        "LLM που θα φορτωθεί στο service",
        options=service_llm_options,
        index=service_llm_options.index(st.session_state.service_llm_model),
        format_func=format_model_option,
        key="service_llm_model",
        help="Η λίστα προέρχεται από το API όταν το service είναι ενεργό ή από το lms CLI όταν το service είναι κλειστό.",
    )
        ...
        if st.button("🚀 Service + model", type="primary", use_container_width=True):
            try:
                if len(service_llm_options) <= 1:
                    raise LMStudioError("Δεν βρέθηκαν LLM models για επιλογή. Άνοιξε το LM Studio τουλάχιστον μία φορά και κατέβασε ένα model.")
                ...
                if not report.get("ok"):
                    raise LMStudioError(report.get("message", "Αποτυχία εκκίνησης service."))
                ...
                st.success(f"Το service είναι ενεργό και χρησιμοποιεί το model `{selected_service_model.identifier}`.")
                st.caption(selected_service_reason)
                st.caption(load_message)
                st.rerun()
            except Exception as exc:
                st.error(f"Αποτυχία εκκίνησης service/model: {exc}")

    with service_btn_col2:
        if st.button("📥 Φόρτωση επιλεγμένου", use_container_width=True):
            try:
                if st.session_state.service_llm_model == AUTO_OPTION:
                    raise LMStudioError("Επίλεξε πρώτα συγκεκριμένο LLM από τη λίστα ή πάτησε 'Service + model' για αυτόματη επιλογή.")
                report = ensure_lmstudio_online(
                    st.session_state.base_url,
                    api_key=st.session_state.api_key,
                    wait_timeout=20,
                ...
                st.session_state.lmstudio_boot_report = report
                if not report.get("ok"):
                    raise LMStudioError(report.get("message", "Αποτυχία σύνδεσης με το service."))

                refresh_models()
                client = build_client()
                chosen_model = get_model_by_identifier(st.session_state.service_llm_model)
                if not chosen_model:
                    raise LMStudioError("Το επιλεγμένο model δεν βρέθηκε μετά την ανανέωση της λίστας.")
                load_message = ensure_model_loaded(client, chosen_model)
                st.session_state.ask_llm_model = chosen_model.identifier
                st.success(load_message)
                st.rerun()
            except Exception as exc:
                st.error(f"Αποτυχία φόρτωσης model: {exc}")

    st.divider()
    st.subheader("📦 Διαθέσιμα μοντέλα")
    st.caption(f"Πηγή λίστας μοντέλων: {st.session_state.models_source}")
    ...
    if not cached_embeddings:
        st.warning(
            "Δεν βρέθηκε embedding model από το LM Studio. "
            "Κατέβασε ή φόρτωσε ένα embedding model και πάτησε ξανά 'Ανανέωση μοντέλων'."
        )
...
with tab_overview:
    st.subheader("Κεντρικός πίνακας")
    ...
    with left:
        st.markdown(
            """
            <div class="mini-card">
                <div class="mini-card-title">Κύριες δυνατότητες</div>
                <div style="color:#0f172a; line-height:1.75;">
                    • Τοπικό RAG πάνω στα αρχεία σου<br>
                    • Επιλογή LLM και αυτόματη εκκίνηση service<br>
                    • Κυλιόμενο πλαίσιο απάντησης με copy/download<br>
                    • Επισύναψη αρχείων στην ερώτηση (PDF, DOCX, κώδικας κ.ά.)<br>
                    • Συλλογές γνώσης με embeddings και αναζήτηση<br>
                    • Καθαρή προβολή πηγών, συλλογών και συνημμένων
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        st.markdown(
            """
            <div class="mini-card">
                <div class="mini-card-title">Γρήγορη ροή χρήσης</div>
                <div style="color:#0f172a; line-height:1.75;">
                    1. Έλεγξε το LM Studio από το sidebar.<br>
                    2. Ανέβασε αρχεία ή φτιάξε συλλογή γνώσης.<br>
                    3. Κάνε ερώτηση και προαιρετικά πρόσθεσε συνημμένα.<br>
                    4. Δες ή αντέγραψε την απάντηση από το πλαίσιο.
                </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with right:
        if st.session_state.latest_result:
            ...
            st.markdown(
                """
                <div class="mini-card">
                    <div class="mini-card-title">Τελευταία απάντηση</div>
                </div>
                """,
                unsafe_allow_html=True,
            )
            st.caption("Ερώτηση: " + ellipsize_middle(latest_question, 90))
            st.info((latest_answer[:220] + "…") if len(latest_answer) > 320 else latest_answer)
            st.caption("Η πλήρης μορφοποιημένη απάντηση εμφανίζεται στην καρτέλα 'Ερώτηση'.")
        else:
            st.info("Δεν υπάρχει ακόμη απάντηση. Κάνε μια ερώτηση από την καρτέλα 'Ερώτηση'.")

with tab_build:
    st.subheader("1) Δημιουργία / Ενημέρωση γνώσης")

    collection_name = st.text_input("Όνομα συλλογής", value="knowledge_base")
    build_embedding_options = [AUTO_OPTION] + embedding_models
    selected_build_embedding_model = st.selectbox(
        "Embedding model από LM Studio",
        options=build_embedding_options,
        index=0,
        format_func=format_model_option,
        key="build_embedding_model",
        help="Άφησέ το σε αυτόματη επιλογή για να βρει και να φορτώσει μόνο του embedding model.",
    )
    ...
    uploaded_files = st.file_uploader(
        ...
        help="Υποστηρίζονται: txt, md, py, json, csv, html, pdf, docx",
    )
    ...
    try:
        ...
        st.caption(f"Αυτόματη επίλυση: `{preview_embedding.identifier}` — {preview_reason}")
    except Exception as exc:
        st.caption(f"Αυτόματη επίλυση embedding model: {exc}")
    ...
    if uploaded_files:
        ...
        c1, c2 = st.columns(2)
        c1.metric("Έγκυρα αρχεία", valid_count)
        c2.metric("Μη έγκυρα αρχεία", len(invalid_files))
        if invalid_files:
            st.warning("Μη υποστηριζόμενα αρχεία: " + ", ".join(invalid_files))

    if st.button("Χτίσιμο / επανεκπαίδευση RAG", type="primary", use_container_width=True):
        try:
            valid_files = [file for file in (uploaded_files or []) if is_supported(file.name)]
            if not valid_files:
                raise ValueError("Πρέπει να ανεβάσεις τουλάχιστον ένα υποστηριζόμενο αρχείο.")
            ...
            with st.spinner("Γίνεται εξαγωγή κειμένου, chunking και δημιουργία embeddings..."):
            ...
            st.success("Η βάση γνώσης δημιουργήθηκε επιτυχώς.")
            ...
            st.error(f"Αποτυχία δημιουργίας συλλογής: {exc}")

with tab_ask:
    st.subheader("2) Ερώτηση πάνω στη βάση γνώσης")
    ...
    selected_collection_option = st.selectbox(
        ...
        help="Μπορείς να απαντήσεις είτε από υπάρχουσα συλλογή είτε μόνο από συνημμένα στην ερώτηση.",
    )
    ...
    with ask_col1:
        selected_llm_model = st.selectbox(
            "LLM model από LM Studio",
            options=llm_options,
            index=llm_options.index(st.session_state.ask_llm_model),
            format_func=format_model_option,
            key="ask_llm_model",
            help="Άφησέ το σε αυτόματη επιλογή για να διαλέξει μόνο του διαθέσιμο LLM.",
        )
    with ask_col2:
        qa_embedding_model = st.selectbox(
            "Embedding model για αναζήτηση",
            options=embedding_options,
            index=0,
            format_func=format_model_option,
            key="qa_embedding_model",
            help="Συνήθως πρέπει να ταιριάζει με το embedding model της συλλογής. Η αυτόματη επιλογή το κάνει μόνη της όταν μπορεί.",
        )

    ask_col3, ask_col4, ask_col5 = st.columns(3)
    with ask_col3:
        top_k = st.slider("Top-K σχετικά αποσπάσματα", min_value=1, max_value=12, value=DEFAULT_TOP_K, step=1)
    with ask_col4:
    ...
    question = st.text_area(
        ...
        value="",
        height=180,
        placeholder="Γράψε εδώ την ερώτησή σου πάνω στα δικά σου δεδομένα...",
    )

    question_attachments = st.file_uploader(
        "Συνημμένα στην ερώτηση",
        type=None,
        accept_multiple_files=True,
        key="qa_attachments",
        help="Μπορείς να επισυνάψεις PDF, DOCX, TXT, JSON, CSV και αρκετούς τύπους αρχείων κώδικα/κειμένου. Τα συνημμένα μπαίνουν κατευθείαν στο prompt σαν επιπλέον context.",
    )
    ...
    if question_attachments:
        ...
        qa_stat1.metric("Έγκυρα συνημμένα", len(valid_question_attachments))
        qa_stat2.metric("Μη υποστηριζόμενα", len(invalid_question_attachments))
        if valid_question_attachments:
            st.caption("Θα αναλυθούν στην ερώτηση: " + ", ".join(file.name for file in valid_question_attachments))
        if invalid_question_attachments:
            st.warning("Μη υποστηριζόμενα συνημμένα: " + ", ".join(invalid_question_attachments))
    ...
    try:
        ...
        st.caption(f"Αυτόματη επίλυση LLM: {exc}")
    ...
    if selected_collection:
        try:
            ...
            st.caption(f"Embedding αναζήτησης: `{preview_qa_embedding.identifier}` — {preview_qa_embedding_reason}")
        except Exception as exc:
            st.caption(f"Αυτόματη επίλυση embedding: {exc}")
    else:
        st.caption("Embedding αναζήτησης: δεν απαιτείται όταν δουλεύεις μόνο με συνημμένα στην ερώτηση.")

    if st.button("Ρώτησε το μοντέλο", type="primary", use_container_width=True):
        try:
            if not question.strip():
                raise ValueError("Η ερώτηση είναι κενή.")
            ...
            if not selected_collection and not attachment_documents:
                raise ValueError("Επίλεξε συλλογή ή ανέβασε τουλάχιστον ένα αναγνώσιμο συνημμένο αρχείο.")
            ...
            embedding_reason = "Δεν απαιτήθηκε embedding model, γιατί η απάντηση βασίστηκε μόνο σε συνημμένα."
            ...
            with st.spinner("Αναζήτηση σχετικών chunks, ανάλυση συνημμένων και δημιουργία απάντησης..."):
            ...
            st.error(f"Αποτυχία απάντησης: {exc}")

    st.markdown("---")
    render_answer_panel(st.session_state.latest_result, panel_key_prefix="ask")

with tab_collections:
    st.subheader("3) Υφιστάμενες συλλογές")

    if not collections:
        st.warning("Δεν υπάρχουν ακόμη συλλογές γνώσης.")
    else:
        search_filter = st.text_input("Φίλτρο συλλογών", value="")
        visible_collections = [
        ...
        if not visible_collections:
            st.info("Δεν βρέθηκαν συλλογές με αυτό το φίλτρο.")
            ...
            try:
                ...
                with st.container(border=True):
                    ...
                    with c1:
                        ...
                        st.caption(f"Πλήρες όνομα: {collection}")
                        st.write(f"Embedding model: `{ellipsize_middle(metadata.get('embedding_model', '-'), 52)}`")
                        st.write(f"Αρχεία: {len(metadata.get('documents', []))}")
                        st.write(f"Chunks: {metadata.get('num_chunks', 0)}")
                        st.write(f"Δημιουργία: {metadata.get('created_at', '-')}")
                        if metadata.get("documents"):
                            shown_docs = [ellipsize_middle(doc, 54) for doc in metadata["documents"]]
                            st.caption("• " + "\n• ".join(shown_docs))
                    with c2:
                        if st.button("🗑️ Διαγραφή", key=f"delete_{collection}", use_container_width=True, help=collection):
                            engine.delete_collection(collection)
                            st.success(f"Η συλλογή '{collection}' διαγράφηκε.")
                            st.rerun()
            except Exception as exc:
                st.error(f"Σφάλμα στη συλλογή '{collection}': {exc}")

st.divider()
st.markdown(
    """
    <div class="footer-note">
        <strong>Σημείωση:</strong> Αν θέλεις πραγματική εκπαίδευση / fine-tuning (π.χ. LoRA ή QLoRA),
        αυτό είναι διαφορετικό pipeline από το RAG. Η παρούσα εφαρμογή σε βοηθά να κάνεις
        το μοντέλο του LM Studio να απαντά πάνω στα δικά σου αρχεία χωρίς να αλλάξεις τα βάρη του μοντέλου.
    </div>
    """,
    unsafe_allow_html=True,
)